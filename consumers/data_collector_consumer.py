...
class DataCollectorConsumer(StreamConsumer):
  ...
  def process_frame(self, frame_data: FrameData) -> bool:
    ...
    try:
      ...
      if self.save_format == "jpg":
        cv2.imwrite(str(frame_path), frame_bgr, [cv2.IMWRITE_JPEG_QUALITY, self.quality])
      else:  # png
  ...
  def stop(self):
    ...
    print(f"[DATA] Effective sample rate: 1/{self.frame_count/self.saved_count:.1f}" if self.saved_count >
          0 else "[DATA] No frames saved")
...
class MonitoringConsumer(StreamConsumer):
  ...
  def __init__(self, consumer_id: str, stream_buffer, stats_interval: float = 5.0):
    super().__init__(consumer_id, stream_buffer)
