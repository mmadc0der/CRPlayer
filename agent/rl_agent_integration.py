...
class AndroidGameEnvironment:
  ...
  def reset(self) -> torch.Tensor:
    ...
    if not self.streamer.is_streaming:
      ...
      while (len(self.frame_buffer) < self.frame_stack and time.time() - start_time < timeout):
        time.sleep(0.1)
  ...
  def step(self, action: torch.Tensor) -> tuple:
    ...
    while (self.last_frame_time == old_time and time.time() - start_wait < timeout):
      time.sleep(0.001)
...
class SimpleRLAgent(nn.Module):
  ...
  def __init__(self, observation_shape: tuple, action_size: int):
    ...
    self.conv_layers = nn.Sequential(nn.Conv2d(observation_shape[0] * observation_shape[1], 32, 8, stride=4), nn.ReLU(),
                                     nn.Conv2d(32, 64, 4, stride=2), nn.ReLU(), nn.Conv2d(64, 64, 3, stride=1),
                                     nn.ReLU(), nn.Flatten())
    ...
    self.policy_head = nn.Sequential(nn.Linear(conv_output_size, 512), nn.ReLU(), nn.Linear(512, action_size))
