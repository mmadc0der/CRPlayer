...
def inspect_data(args):
  ...
  for dataset in data_info['datasets']:
    ...
    table.add_row(dataset['project_name'], dataset['dataset_name'], dataset['target_type'],
                  str(progress.get('labeled', 0)), str(dataset['num_classes']))
...
def run_experiment(args):
  ...
  config = ExperimentConfig(dataset_id=args.dataset_id,
...
def auto_label(args):
  ...
  config = DatasetConfig()
  pipeline = AutoLabelingPipeline(model_path=args.model_path,
                                  model_type=args.model_type,
                                  num_classes=args.num_classes,
                                  config=config)

  # Process corpus
  results = pipeline.process_corpus(image_paths=image_paths,
...
def run_distillation(args):
  ...
  config = DistillationConfig(teacher_model_path=args.teacher_model_path,
  ...
  # Load and normalize dataset using same logic as experiment_runner
  inspector = DatasetInspector(data_config)
  download = inspector.download_dataset(args.dataset_id)

  df = download['dataframe']
  class_to_idx = download['metadata']['class_to_idx']
  idx_to_class = download['metadata']['idx_to_class']
  num_classes = download['metadata']['num_classes']

  # Normalize labels to contiguous 0..C-1
  all_ids = sorted(idx_to_class.keys())
  id_to_idx = {cid: i for i, cid in enumerate(all_ids)}
  df = df.copy()
  df['class_id_norm'] = df['class_id'].map(id_to_idx)

  records = []
  for row in df.to_dict('records'):
    r = dict(row)
    r['single_label_class_id'] = int(row['class_id_norm']) if row['class_id_norm'] is not None else 0
    records.append(r)

  train_loader, val_loader, test_loader = create_data_loaders(records,
  ...
  console.print(f"[green]Student best_val_f1: {results['best_val_f1']:.4f}[/green]")

  # Compare on test
  comparison = pipeline.compare_models(test_loader)
  console.print("[green]Teacher vs Student comparison complete.[/green]")
...
def main():
  ...
  autolabel_parser.add_argument('--active-learning-samples',
                                type=int,
                                default=100,
                                help='Number of active learning samples')
