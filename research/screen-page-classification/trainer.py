        ...
        if 'per_class_metrics' in metrics:
            ...
            for i, class_name in enumerate(metrics['class_names']):
                per_class_table.add_row(
                    class_name,
                    f"{metrics['per_class_metrics']['precision'][i]:.3f}",
...
class DistillationTrainer(ClassificationTrainer):
    ...
    def train_with_distillation(
        ...
        for epoch in range(num_epochs):
            ...
            train_metrics = self.train_epoch_distillation(
                train_loader, optimizer, teacher_model, device
            )
