...
class AdvancedDistillationLoss(nn.Module):
  ...
  def forward(self,
              ...
              teacher_embeddings: Optional[torch.Tensor] = None) -> Dict[str, torch.Tensor]:
...
class DistillationPipeline:
  ...
  def __init__(self, config: DistillationConfig, data_config: DatasetConfig):
    ...
    self.student_model = ModelFactory.create_model(config.student_model_type,
                                                   num_classes=config.num_classes,
                                                   pretrained=True)

    # Initialize advanced loss
    self.distillation_loss = AdvancedDistillationLoss(temperature=config.temperature,
                                                      alpha=config.alpha,
                                                      beta=config.beta,
                                                      gamma=config.beta,
  ...
  def _load_teacher_model(self) -> BaseClassifier:
    ...
    teacher_model = ModelFactory.create_model(self.config.teacher_model_type,
  ...
  def train_student(self,
    ...
    # Setup optimizer
    optimizer = torch.optim.AdamW(self.student_model.parameters(), lr=learning_rate, weight_decay=weight_decay)
    ...
    for epoch in range(num_epochs):
      # Train epoch
      train_metrics = self._train_epoch_distillation(train_loader, optimizer, device)

      # Validate epoch
      val_metrics = self._validate_epoch_distillation(val_loader, device)

      # Log metrics
      self._log_metrics(epoch, train_metrics, val_metrics)

      # Store history
      training_history.append({'epoch': epoch, 'train': train_metrics, 'val': val_metrics})
    ...
    return {'best_val_f1': best_val_f1, 'training_history': training_history}

  def _train_epoch_distillation(self, train_loader, optimizer: torch.optim.Optimizer,
    ...
    with Progress(TextColumn("[progress.description]{task.description}"),
      ...
      for data, target in train_loader:
        ...
        # Forward pass (student needs grad)
  ...
  def _validate_epoch_distillation(self, val_loader, device: torch.device) -> Dict[str, float]:
    """Validate one epoch."""
  ...
  def compare_models(self, test_loader, device: torch.device = None) -> Dict[str, Any]:
    ...
    teacher_precision, teacher_recall, teacher_f1, _ = precision_recall_fscore_support(targets,
                                                                                       teacher_predictions,
                                                                                       average='weighted',
                                                                                       zero_division=0)

    student_precision, student_recall, student_f1, _ = precision_recall_fscore_support(targets,
                                                                                       student_predictions,
                                                                                       average='weighted',
                                                                                       zero_division=0)
    ...
    comparison = {
      ...
      'compression_ratio':
      teacher_info['parameters']['total_parameters'] / student_info['parameters']['total_parameters'],
  ...
  def _display_comparison(self, comparison: Dict[str, Any]):
    ...
    table.add_row("Accuracy", f"{comparison['teacher']['accuracy']:.4f}", f"{comparison['student']['accuracy']:.4f}",
                  f"{acc_diff:+.4f}")

    # F1 Score
    f1_diff = comparison['student']['f1_score'] - comparison['teacher']['f1_score']
    table.add_row("F1 Score", f"{comparison['teacher']['f1_score']:.4f}", f"{comparison['student']['f1_score']:.4f}",
                  f"{f1_diff:+.4f}")

    # Parameters
    param_diff = comparison['student']['parameters'] - comparison['teacher']['parameters']
    table.add_row("Parameters", f"{comparison['teacher']['parameters']:,}", f"{comparison['student']['parameters']:,}",
                  f"{param_diff:+,}")

    # Model Size
    size_diff = comparison['student']['model_size_mb'] - comparison['teacher']['model_size_mb']
    table.add_row("Size (MB)", f"{comparison['teacher']['model_size_mb']:.2f}",
                  f"{comparison['student']['model_size_mb']:.2f}", f"{size_diff:+.2f}")
...
def main():
  ...
  config = DistillationConfig(teacher_model_path="/path/to/teacher_model.pth",
