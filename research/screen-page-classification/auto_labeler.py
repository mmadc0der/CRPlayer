...
class ConfidenceFilter:
  ...
  def filter_predictions(self, predictions: np.ndarray, confidences: np.ndarray,
...
class AutoLabeler:
  ...
  def __init__(self,
    ...
    # Enable AMP by default on CUDA for faster inference
    self.use_amp = (torch.cuda.is_available() if use_amp is None else use_amp)
    ...
    self.model.eval()
    # Optimize CuDNN for fixed-size inference
    ...
    with torch.inference_mode():
      with Progress(TextColumn("[progress.description]{task.description}"),
        ...
        for batch_images, batch_paths in dataloader:
          ...
          # Extract features if needed (use logits)
          if return_features:
            batch_features = outputs.float().cpu().numpy()
  ...
  def auto_label_corpus(self,
    ...
    filtered_results = self.confidence_filter.filter_predictions(results['predictions'], results['confidences'],
                                                                 results['image_paths'])
    ...
    return {'filtered_results': filtered_results, 'summary': summary, 'raw_results': results}
  ...
  def select_for_active_learning(self, image_paths: List[str], num_samples: int = 100) -> List[str]:
    """Select samples for active learning."""
    ...
    # Select samples
    selected_paths = self.active_learning_selector.select_samples(results['predictions'], results['confidences'],
                                                                  results['features'], results['image_paths'],
                                                                  num_samples)
...
class AutoLabelingPipeline:
  ...
  def __init__(self, model_path: str, model_type: str, num_classes: int, config: DatasetConfig):
    self.config = config
  ...
  def process_corpus(self,
                     ...
                     output_dir: str,
                     confidence_threshold: float = 0.9,
                     active_learning_samples: int = 100) -> Dict[str, Any]:
    ...
    auto_label_results = self.auto_labeler.auto_label_corpus(image_paths,
    ...
    active_learning_samples = self.auto_labeler.select_for_active_learning(image_paths, active_learning_samples)
    ...
    class_filtered = self.auto_labeler.filter_by_class(image_paths, list(range(self.num_classes)))
