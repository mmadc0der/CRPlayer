...
class DeviceManager:
  ...
  def create_tensor(self, data: Any, dtype: torch.dtype = torch.float32, requires_grad: bool = False) -> torch.Tensor:
    """
  ...
  def get_memory_info(self) -> Dict[str, float]:
    ...
    if torch.cuda.is_available():
      ...
      info.update({'allocated_gb': 0.0, 'reserved_gb': 0.0, 'total_gb': 0.0, 'available_gb': 0.0})
  ...
  def get_optimal_batch_size(self, model: nn.Module, input_shape: Tuple[int, ...], max_batch_size: int = 64) -> int:
    """
...
def ensure_tensor_on_device(tensor: torch.Tensor,
                            device: torch.device,
...
def save_model_with_device_info(model: nn.Module, path: Union[str, Path], device_manager: DeviceManager, **kwargs):
  """
...
def load_model_with_device_info(path: Union[str, Path], model: nn.Module, device_manager: DeviceManager) -> nn.Module:
  """
