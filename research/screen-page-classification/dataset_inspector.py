...
class DatasetAnalyzer:
  ...
  def _generate_visualizations(self, df: pd.DataFrame, balance_analysis: Dict[str, Any], output_path: Path):
    ...
    plt.bar(['Imbalance Ratio'], [imbalance_ratio],
            color='red' if imbalance_ratio > 10 else 'orange' if imbalance_ratio > 5 else 'green')
  ...
  def _generate_recommendations(self, report: Dict[str, Any]) -> List[str]:
    ...
    if balance_quality == "Poor":
      recommendations.append(
        "⚠️  High class imbalance detected. Consider using class weights, oversampling, or data augmentation.")
    ...
    if total_samples > 10000:
      recommendations.append(
        "🚀 Large dataset detected. Consider using efficient training strategies and distributed training.")
