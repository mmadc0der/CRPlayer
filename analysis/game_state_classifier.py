...
class GameStateDataset(Dataset):
  ...
  def __init__(self, annotation_file: str, transform=None):
    ...
    self.state_to_idx = {'menu': 0, 'loading': 1, 'battle': 2, 'final': 3}
...
class TinyGameStateClassifier(nn.Module):
  ...
  def __init__(self, num_classes=4, input_size=(224, 224)):
    ...
    # Classifier
    self.classifier = nn.Sequential(nn.Dropout(0.5), nn.Linear(64, 32), nn.ReLU(inplace=True), nn.Dropout(0.3),
...
def train_game_state_classifier(annotation_file: str, val_split: float = 0.2, batch_size: int = 16, epochs: int = 20):
  """
  ...
  def classify_frame(tensor, pts, timestamp):
    ...
    try:
      ...
      if pts % 30 == 0:
        ...
        prob_str = " | ".join([f"{state}: {prob:.3f}" for state, prob in zip(state_names, probs[0])])
...
if __name__ == "__main__":
  ...
  if command == "train":
    ...
    model, trainer, best_acc = train_game_state_classifier(annotation_file=annotation_file, epochs=25, batch_size=16)
