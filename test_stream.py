...
class StreamingTester:
  ...
  def _print_results(self, streamer):
    ...
    print()
    print(
      f"Latency - Avg: {avg_latency:.1f}ms, Min: {self.min_latency * 1000:.1f}ms, Max: {self.max_latency * 1000:.1f}ms")
...
def test_gpu_acceleration():
  ...
  resized = torch.nn.functional.interpolate(test_tensor.unsqueeze(0), size=(224, 224), mode="bilinear").squeeze(0)
