...
class SharedStreamBuffer:
  ...
  def __init__(self, max_buffer_size: int = 100):
    ...
    # Statistics
    self.stats = {'frames_produced': 0, 'frames_consumed': 0, 'memory_cleanups': 0, 'active_consumers': 0}
  ...
  def _cleanup_inactive_consumers(self):
    ...
    with self._lock:
      inactive_consumers = [consumer_id for consumer_id, iterator in self.consumers.items() if not iterator.is_alive()]
