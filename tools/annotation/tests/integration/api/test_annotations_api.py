...
from tests.fixtures.factories import (
  ProjectDataFactory,
  DatasetDataFactory,
  SaveRegressionRequestFactory,
  SaveSingleLabelRequestFactory,
...
class TestAnnotationsAPI:
  ...
  def setup_project_and_dataset(self, client: FlaskClient, target_type_id: int = 1):
    ...
    project_response = client.post("/api/projects", data=json.dumps(project_data), content_type="application/json")
    project_id = project_response.get_json()["id"]

    # Create dataset
    dataset_data = DatasetDataFactory(target_type_id=target_type_id)
    dataset_response = client.post(f"/api/projects/{project_id}/datasets",
  ...
  def test_save_regression_success(self, client: FlaskClient):
    ...
    request_data = SaveRegressionRequestFactory(dataset_id=dataset_id, frame_idx=0, value=42.5)

    with patch("services.annotation_service.AnnotationService.save_regression") as mock_save, patch(
        "services.session_service.SessionService.get_frame_by_idx") as mock_get_frame:
      mock_save.return_value = {"regression_value": 42.5, "status": "labeled"}
      mock_get_frame.return_value = {"frame_id": "frame_001", "ts_ms": 1000}

      response = client.post("/api/annotations/regression",
  ...
  def test_save_single_label_with_class_id(self, client: FlaskClient):
    ...
    request_data = SaveSingleLabelRequestFactory(dataset_id=dataset_id, frame_idx=0, class_id=1)

    with patch("services.annotation_service.AnnotationService.save_single_label") as mock_save, patch(
        "services.session_service.SessionService.get_frame_by_idx") as mock_get_frame:
      mock_save.return_value = {"single_label_class_id": 1, "status": "labeled"}
      mock_get_frame.return_value = {"frame_id": "frame_001", "ts_ms": 1000}

      response = client.post("/api/annotations/single_label",
                             data=json.dumps(request_data),
                             content_type="application/json")
  ...
  def test_save_single_label_with_category_name(self, client: FlaskClient):
    ...
    with patch("services.annotation_service.AnnotationService.save_single_label") as mock_save, patch(
        "services.session_service.SessionService.get_frame_by_idx") as mock_get_frame, patch(
          "db.classes.get_or_create_dataset_class") as mock_get_class:
  ...
  def test_save_multilabel_success(self, client: FlaskClient):
    ...
    request_data = SaveMultilabelRequestFactory(dataset_id=dataset_id, frame_idx=0, class_ids=[1, 2, 3])

    with patch("services.annotation_service.AnnotationService.save_multilabel") as mock_save, patch(
        "services.session_service.SessionService.get_frame_by_idx") as mock_get_frame:
      mock_save.return_value = {"class_ids": [1, 2, 3], "status": "labeled"}
      mock_get_frame.return_value = {"frame_id": "frame_001", "ts_ms": 1000}

      response = client.post("/api/annotations/multilabel",
                             data=json.dumps(request_data),
                             content_type="application/json")
  ...
  def test_save_annotation_with_override_settings(self, client: FlaskClient):
    ...
    request_data = SaveRegressionRequestFactory(dataset_id=dataset_id,
                                                frame_idx=0,
                                                value=42.5,
  ...
  def test_frame_id_vs_frame_idx_resolution(self, client: FlaskClient):
    ...
    request_data = SaveRegressionRequestFactory(
      dataset_id=dataset_id,
      frame_idx=0,
      value=42.5  # Using frame_idx instead of frame_id
    )
  ...
  def test_malformed_json_handling(self, client: FlaskClient):
    ...
    response = client.post(
      "/api/annotations/regression",
      data='{"invalid": json}',
      content_type="application/json"  # Malformed JSON
    )
  ...
  def test_missing_content_type(self, client: FlaskClient):
    ...
    response = client.post("/api/annotations/regression", data=json.dumps(request_data)
...
class TestAnnotationRetrievalAPI:
  ...
  def test_get_annotation_success(self, client: FlaskClient):
    ...
    with patch("services.annotation_service.AnnotationService.get_annotation_db") as mock_get:
      mock_annotation = {"id": 1, "dataset_id": 1, "frame_id": 456, "status": "labeled", "regression_value": 42.5}
