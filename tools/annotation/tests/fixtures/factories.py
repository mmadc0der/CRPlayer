...
class SessionMetadataFactory(factory.DictFactory):
  ...
  fps = 30
  device_info = factory.Dict({"model": "TestDevice", "os": "TestOS 1.0"})
...
class DatasetSessionSettingsFactory(factory.DictFactory):
  ...
  hotkeys = factory.Dict({"1": "menu", "2": "loading", "3": "battle", "4": "final"})
...
def create_session_with_frames(session_id: str = None, frame_count: int = 5) -> Dict[str, Any]:
  ...
  return {"session_id": session_id, "metadata": metadata, "frames": frames, "root_path": f"/test/data/{session_id}"}
...
def create_annotation_scenario(session_count: int = 2, frames_per_session: int = 10) -> Dict[str, Any]:
  ...
  for i in range(session_count):
    session = create_session_with_frames(session_id=f"scenario_session_{i+1:03d}", frame_count=frames_per_session)
    sessions.append(session)

  return {"project": project_data["project"], "dataset": project_data["datasets"][0], "sessions": sessions}
