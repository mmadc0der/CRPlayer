...
from db.projects import (
    ...
    list_datasets, create_dataset, get_dataset, update_dataset, 
    delete_dataset, dataset_progress, get_dataset_by_name
)
...
class TestDatasetsDatabase:
    ...
    def test_create_dataset_success(self, temp_db):
        ...
        cursor = temp_db.execute(
            "SELECT project_id, name, description, target_type_id FROM datasets WHERE id = ?",
            (dataset_id,)
        )
        row = cursor.fetchone()
        assert row is not None
    ...
    def test_create_dataset_duplicate_name_in_project_fails(self, temp_db):
        ...
        create_dataset(temp_db, project_id, dataset_data["name"], dataset_data["description"], dataset_data["target_type_id"])
    ...
    def test_list_datasets_with_data(self, temp_db):
        ...
        id1 = create_dataset(temp_db, project_id, dataset1_data["name"], dataset1_data["description"], dataset1_data["target_type_id"])
        id2 = create_dataset(temp_db, project_id, dataset2_data["name"], dataset2_data["description"], dataset2_data["target_type_id"])
