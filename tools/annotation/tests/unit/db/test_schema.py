...
class TestDatabaseSchema:
  ...
  def test_init_db_creates_all_tables(self, temp_db):
    ...
    cursor = temp_db.execute("SELECT name FROM sqlite_master WHERE type='table' ORDER BY name")
  ...
  def test_init_db_creates_target_types(self, temp_db):
    ...
    expected_types = [(0, "Regression"), (1, "SingleLabelClassification"), (2, "MultiLabelClassification")]
  ...
  def test_projects_table_structure(self, temp_db):
    ...
    expected_columns = {"id": "INTEGER", "name": "TEXT", "description": "TEXT", "created_at": "DATETIME"}
  ...
  def test_sessions_table_structure(self, temp_db):
    ...
    expected_columns = {"id": "INTEGER", "session_id": "TEXT", "root_path": "TEXT", "metadata_json": "TEXT"}
  ...
  def test_frames_table_structure(self, temp_db):
    ...
    expected_columns = {"id": "INTEGER", "session_id": "INTEGER", "frame_id": "TEXT", "ts_ms": "INTEGER"}
  ...
  def test_check_constraints(self, temp_db):
    ...
    with pytest.raises(sqlite3.IntegrityError):
      temp_db.execute("INSERT INTO frames(session_id, frame_id, ts_ms) VALUES (1, 'test', -1000)")

  def test_indexes_created(self, temp_db):
    """Test that necessary indexes are created."""
    cursor = temp_db.execute("SELECT name FROM sqlite_master WHERE type='index' AND sql IS NOT NULL")
