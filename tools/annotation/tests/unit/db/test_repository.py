...
from db.repository import (
    transaction, upsert_session, upsert_frame, get_session_db_id, 
    get_frame_db_id, upsert_regression, upsert_single_label,
    replace_multilabel_set, ensure_membership, set_annotation_status,
...
class TestDatabaseRepository:
    ...
    def test_upsert_session_creates_new_session(self, temp_db):
        ...
        cursor = temp_db.execute(
            "SELECT session_id, root_path, metadata_json FROM sessions WHERE id = ?",
            (session_db_id,)
        )
        row = cursor.fetchone()
    ...
    def test_upsert_session_updates_existing_session(self, temp_db):
        ...
        cursor = temp_db.execute(
            "SELECT root_path, metadata_json FROM sessions WHERE id = ?",
            (session_db_id1,)
        )
        row = cursor.fetchone()
    ...
    def test_upsert_frame_creates_new_frame(self, temp_db):
        ...
        cursor = temp_db.execute(
            "SELECT frame_id, ts_ms FROM frames WHERE id = ?",
            (frame_db_id,)
        )
        row = cursor.fetchone()
        assert row is not None
