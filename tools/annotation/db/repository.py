...
def ensure_membership(conn: sqlite3.Connection, dataset_id: int, frame_db_id: int, default_status: str = 'unlabeled') -> None:
    conn.execute(
        """
...
def list_frames_with_annotations(
    ...
    base_sql = (
        ...
        WHERE f.session_id = ?
        """
        + (" AND a.status = 'labeled'" if labeled_only else "")
        + " ORDER BY f.frame_id"
    )
