"""
Deterministic helpers to resolve session and frame paths (raw-only).
Avoids expensive recursive scans and enforces canonical layout:
- Sessions live under data/raw/<session_id>
...
def resolve_session_dir(session_manager: SessionManager,
                        session_id: str,
                        explicit_session_dir: Optional[Path] = None) -> Optional[Path]:
  """Resolve a session directory by id (raw-only).
    Precedence:
    1) explicit_session_dir if it exists
    2) data/raw/<session_id>
    """
...
def resolve_frame_relative_path(
  ...
  """Resolve a frame's relative path inside dataset manifests using a deterministic search order (raw-only).
  ...
  try:
    # Determine if session_dir is under raw to build relative base accordingly
    sd_str = str(session_dir)
    if str(raw_root) in sd_str:
...
def resolve_frame_absolute_path(
  ...
  """Resolve a frame to an absolute Path using the same deterministic order as relative resolver (raw-only)."""
  fname = Path(filename)
  basename = fname.name
  raw_root = session_manager.raw_dir / session_id
